...
test("Highlight selected date", function() {
	...
	!ok($('.dp-selected'), "No date is highlighted when set to false");
});
...
test("Highlight today", function() {
	...
	!ok($('.dp-today'), "No date is highlighted when set to false");
});
...
test("Show calendar", function() {
	...
	!ok($('#dp-calendar'), "No calendar when set to false");
});
...
test("Item text", function() {
	...
	function format(el, format) {
		return moment(el.attr("data-moment")).format(format)
			.replace("<br/>", "").replace("<br>",""); // TODO Use regex to parse html tags 
	}
	...
	equal(itemEl.text(), format(itemEl, options.text), 
		"Item text correct when using default");
	equal(selectedItemEl.text(), format(selectedItemEl, options.textSelected), 
	...
	options = getOptions(init({text:'Do<br>ddd',textSelected:'Do, MMMM YYYY<br>dddd'})),
		itemEl = $('.dp-item:first'),
		selectedItemEl = $('.dp-selected');
	equal(itemEl.text(), format(itemEl, 'Do<br>ddd'), 
		"Item text correct when using custom value");
	equal(selectedItemEl.text(), format(selectedItemEl, 'Do, MMMM YYYY<br>dddd'), 
...
test("Size", function() {
	...
	!ok($('.dp-item-sm') || $('.dp-item-lg'), "Item set correct when using default");
	!ok($('.dp-nav-sm') || $('.dp-nav-lg'), "Nav item set correct when using default");
	init({size:'normal'})
	ok($('.datepaginator'), "Correctly set .datepaginator when size set as normal ");
	!ok($('.dp-item-sm') || $('.dp-item-lg'), "Item set correct when size set as normal");
	!ok($('.dp-nav-sm') || $('.dp-nav-lg'), "Nav item set correct when size set as normal");
	init({size:'small'});
...
test("Square Edges", function() {
	init();
	!ok($('.dp-nav-sqauare-edges'), "Using rounded edges by default");
	init({squareEdges:false});
	!ok($('.dp-nav-sqauare-edges'), "Square edges not used when squareEdges set to false");
